...
#let _MESSAGES = (
  ...
  "template.guard-type-is-bib-field": "gb7714: 守卫 `type={value}` 里的 `type` 现在指*bib 的 `type` 字段*（报告种类、学位类型），不再是文献类型标识码。。要按标识码判，写 `mark={value}`；要按载体码判，写 `medium=OL`。（真想匹配 `type` 字段值为「{value}」的条目，这条守卫写不出来——那个值恰好是标识码，无法与误用区分。）",
  "template.guard-expected-field": "gb7714: 守卫表达式里应是「字段=值」或光秃秃一个字段名（空非空），这里缺字段名（写法如 `<mark=M => …>` / `<doi => …>`）",
  "template.guard-ambiguous-or": "gb7714: 守卫里 `{field}=…` 的 `?` 后面跟了一个裸 token 名 `{token}`——两种读法都说得通，写明白一点：\n① 想问「这条目有没有 `{token}`」（空非空），把它包进尖括号：`{field}=… ? <{token}> => …`；\n② 想把 `{token}` 当成 `{field}` 的候选*取值*，把它包进花括号（verbatim）：`{field}=…?{{token}} => …`。\n（`?` 在守卫里身兼两职：算符「或」与同字段的「值或」。裸值不是 token 名时不会有这个歧义。）",
  "template.guard-expected-value": "gb7714: 守卫字段 `{field}` 的 `=` 后缺值（值带空格或冒号请用花括号 verbatim，如 `t={10:30}`）",
  "template.guard-nested-arrow": "gb7714: 守卫的 `<…>` 分组里不能再出现 `=>`（分组只作布尔括号，不是嵌套守卫组）",
  "template.guard-arrow-outside-group": "gb7714: `=>` 只能作 `<…>` 守卫组里的谓词与组体的分界（写法如 `<mark=M => {图书}>`），组外没有这个算符",
  "template.guard-multiple-arrows": "gb7714: 同一个 `<…>` 守卫组里出现了不止一个 `=>`——守卫组的形态是「一个谓词、一个组体」，组体里要再分支就再嵌一层 `<…>`",
  "template.guard-trailing": "gb7714: 守卫表达式解析后有多余内容——检查算符（`& ? !`）与 `<…>` 分组是否配对",
  ...
  "separator.unknown-lang-key": "gb7714: 分隔符的多语言字典收到未知键 \"{key}\"；合法键: {keys}（条目语言码，加 rest 兜底档——未点名的语言走本参数的预设值）。",

  "et-al.unknown-key": "gb7714: {param} 的字典收到未知键 \"{key}\"；合法键有两套——角色键 {roles}（截断发生的四个位置），或语言键 {langs}（条目语言码）；两套都可以加 rest 兜底档，但不能并列在同一层。",
  "et-al.unknown-lang-key": "gb7714: {param} 的语言字典收到未知键 \"{key}\"；合法键: {keys}（条目语言码，加 rest 兜底档）。",
  "et-al.mixed-keys": "gb7714: {param} 的字典不能把角色键（{roles}）与语言键（{langs}）并列在同一层；语言分设要写进角色的值里，如 (editor: (en: 5, rest: 4), rest: 4)。",
  "et-al.no-role-fallback": "gb7714: {param} 的角色字典既没有键 \"{role}\"，也没有兜底键 rest；补上其中之一。",
  "et-al.no-lang-fallback": "gb7714: {param} 的语言字典没有点到本条目的语言（{lang}），也没有兜底键 rest；补上其中之一。",
